...
Labyrinth create_test_labyrinth() {
  Labyrinth labyrinth = {0};
  labyrinth.rows = 3;
  labyrinth.cols = 3;
  create_matrix(&labyrinth.v_w, 3, 3);
  create_matrix(&labyrinth.h_w, 3, 3);
  create_matrix(&labyrinth.pass, 3, 3);
  return labyrinth;
}
...
START_TEST(test_q_agent_choose_action) {
  ...
  Labyrinth labyrinth = create_test_labyrinth();
  Point cur = {1, 1};
  int action = -1;

  Point next = q_agent_choose_action(&agent, cur, &labyrinth, &action);
...
START_TEST(test_q_agent_find_path_simple) {
  ...
  bool found = q_agent_find_path(&agent, &labyrinth, start, &pass);
...
START_TEST(test_visualize_q_values_file_output) {
  QLearningAgent agent = {0};
  Labyrinth labyrinth = {.rows = 2, .cols = 2};
  create_matrix(&labyrinth.v_w, 2, 2);
  create_matrix(&labyrinth.h_w, 2, 2);
  ...
  visualize_q_values(f, &agent, &labyrinth);
  fclose(f);
