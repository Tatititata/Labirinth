...
START_TEST(test_parse_maze_file_basic_manual) {
  ...
  Maze maze = {0};
  bool success = parse_maze_file(f, &maze, 'm');
  fclose(f);
  remove(TEMP_FILENAME);

  ck_assert_msg(success, "Parsing should succeed.");
  ck_assert_int_eq(maze.rows, 3);
  ck_assert_int_eq(maze.cols, 3);
  ck_assert_ptr_nonnull(maze.v_w);
  ck_assert_ptr_nonnull(maze.h_w);
  ck_assert_ptr_nonnull(maze.pass);

  ck_assert_int_eq(maze.v_w[1][0], 1);
  ck_assert_int_eq(maze.h_w[2][2], 1);

  destroy_maze(&maze);
}
END_TEST

START_TEST(test_parse_maze_file_fail_on_wrong_size) {
  const char *maze_data = "a b\n";
  FILE *f = fopen(TEMP_FILENAME, "w");
  fputs(maze_data, f);
  fclose(f);
  ...
  ck_assert_ptr_null(maze.v_w);
}
END_TEST

START_TEST(test_parse_maze_file_fail_on_wrong_data) {
  const char *maze_data = "3 3\n\n 1 1 0\n 1 1\n";
  FILE *f = fopen(TEMP_FILENAME, "w");
  fputs(maze_data, f);
  fclose(f);
...
START_TEST(test_parse_maze_file_with_random_spaces_and_newlines) {
...
Suite *parser_suite(void) {
  ...
  tcase_add_test(tc, test_parse_maze_file_basic_manual);
  tcase_add_test(tc, test_parse_maze_file_fail_on_wrong_size);
  tcase_add_test(tc, test_load_matrix_valid);
  tcase_add_test(tc, test_load_matrix_invalid_data);
  tcase_add_test(tc, test_parse_maze_file_with_random_spaces_and_newlines);
  tcase_add_test(tc, test_parse_maze_file_fail_on_wrong_data);
